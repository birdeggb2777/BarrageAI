    ...
    for i in range(0,24000,power):      
        ...
        if Balltimes>=(i)*power:
            ...
            if math.sqrt(((They_Array[i].tempX()-My_X)**2+(They_Array[i].tempY()-My_Y)**2))<=distance:
                if math.sqrt(((They_Array[i].tempX()-(My_X-danger))**2+(They_Array[i].tempY()-(My_Y))**2))<=distance:
                    Now_Move=2
                elif math.sqrt(((They_Array[i].tempX()-(My_X+danger))**2+(They_Array[i].tempY()-(My_Y))**2))<=distance:
                    Now_Move=1
                else:
                    Now_Move=0
                Now_flag=1
            if math.sqrt(((They2_Array[i].tempX()-My_X)**2+(They2_Array[i].tempY()-My_Y)**2))<=distance:
                if math.sqrt(((They2_Array[i].tempX()-(My_X-danger))**2+(They2_Array[i].tempY()-(My_Y))**2))<=distance:
                    Now_Move=2
                elif math.sqrt(((They2_Array[i].tempX()-(My_X+danger))**2+(They2_Array[i].tempY()-(My_Y))**2))<=distance:
                    Now_Move=1
                else:
                    Now_Move=0
                Now_flag=1
            ...
            if 1024>They_Array[i].they_X>0 and 768>They_Array[i].they_Y>0:
                pg.draw.ellipse(screen,yellow,[They_Array[i].they_X+BallSize,They_Array[i].they_Y+BallSize,BallSize,BallSize])
            if 1024>They2_Array[i].they_X>0 and 768>They2_Array[i].they_Y>0:
                pg.draw.ellipse(screen,yellow,[They2_Array[i].they_X+BallSize,They2_Array[i].they_Y+BallSize,BallSize,BallSize])
